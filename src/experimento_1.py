...
from mealpy.evolutionary_based.DE import OriginalDE as BaseDE
...
def run_algorithm(algoritmo, problem, pop_size, epoch):
    if algoritmo == "GA":
        ...
        model = BaseDE(epoch=epoch, pop_size=pop_size)
    else:
...
def main():
    ...
    st.set_page_config(page_title="Otimização com GA e DE", page_icon="🧊", layout='wide')

    st.title("Otimização com GA e DE utilizando Mealpy")
    
    nruns = st.sidebar.number_input("Número de Execuções", min_value=1, max_value=50, value=20)
    maxfes = st.sidebar.number_input("Número de Avaliações de Função", min_value=1000, max_value=100000, value=50000)
    
    pop_size = st.sidebar.slider("Tamanho da População", 10, 200, 50)
    ...
    with col1:
        if st.button("Executar"):
            ...
            with col2:
                ...
                colors = {"GA": "dodgerblue", "DE": "goldenrod"}

                # Exibindo gráfico de barras estacadas com os resultados para cada algoritmo X função
                fig = px.bar(df_unificado, x="Problema", y="ratio", title=f"Razão entre as funções com e sem shift para os algoritmos GA e DE", text="ratio", color="Algoritmo", labels={"ratio": "Razão", "Problema": "Função"}, barmode="group", color_discrete_map=colors)
                ...
                fig = px.bar(medias, x="Algoritmo", y="geomeans", title="Média Geométrica das razões entre as funções com e sem shift para os algoritmos GA e DE", color="Algoritmo",text="geomeans", labels={"Algoritmo": "Algoritmo", "geomeans": "Média Geométrica"}, color_discrete_map=colors)
                fig.update_yaxes(tickformat=".2e")
